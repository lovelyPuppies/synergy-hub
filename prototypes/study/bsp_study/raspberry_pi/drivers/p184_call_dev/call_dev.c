...
static loff_t call_llseek(struct file *filp, loff_t off, int whence) {
  printk("call llseek -> of: %08X, whence : %08X\n", (unsigned int)off, whence);
  return 0x23;
}

static ssize_t call_read(struct file *filp, char *buf, size_t count,
                         loff_t *f_pos) {
...
static struct file_operations call_fops = {
    ...
    .read = call_read,
    .write = call_write,
    .unlocked_ioctl = call_ioctl, // 📝 '.ioctl = call_ioctl' is legacy.
    .open = call_open,
