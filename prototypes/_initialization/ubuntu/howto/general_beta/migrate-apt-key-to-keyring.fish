...
function fix_gpg_key
    ...
    echo "deb [signed-by=/etc/apt/keyrings/$repo_file.gpg] $repo_url $(lsb_release -cs) main" | sudo tee /etc/apt/sources.list.d/$repo_file.list >/dev/null
...
sudo apt update; and sudo apt install microsoft-edge-stable
