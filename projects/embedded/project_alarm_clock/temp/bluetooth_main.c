/* USART2 usb to serial debug   PA2 USART2_TX, PA3 USART2_TX
   USART6 bluetooth & wifi Seiral  PC6 USART6_TX , PC7 USART6 RX */
...
int main(void) {
  ...
  while (1) {
    if (rx2Flag) {
      printf("recv2 : %s\r\n", rx2Data);
      rx2Flag = 0;
      //	    HAL_UART_Transmit(&huart6, (uint8_t *)buf, strlen(buf),
      // 0xFFFF);
    }
    if (btFlag) {
      //		printf("bt : %s\r\n",btData);
