...
Terminal 1
  ...
  sudo chmod 777 /dev/minor_led
  sudo chmod 777 /dev/minor_key


  sudo insmod /mnt/host/drivers/minor_dev.ko
  /mnt/host/drivers/minor_app 0x55

  sudo rmmod minor_dev
...
ssize_t minor0_write(struct file *filp, const char *buf, size_t count,
                     loff_t *f_pos) {
...
int minor_open(struct inode *inode, struct file *filp) {
  // f_op는 해당 파일 핸들(filp)이 사용할 file_operations 구조체를 가리킴
  ...
  if (filp->f_op && filp->f_op->open) // 🚣 if not null
