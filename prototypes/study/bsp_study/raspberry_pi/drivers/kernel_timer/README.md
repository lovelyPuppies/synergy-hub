...
- **크로스 컴파일(llvm/clang) 환경 구축**  
  LLVM/clang 스택을 사용하여, 임베디드/크로스 환경에서 빌드 및 배포가 용이하도록 설정합니다.

- **Makefile 구조화 및 위임**

  - 상위 `Makefile`에서 빌드, 클린, 배포(install) 등 공통 타겟을 정의하고,  
    하위 `app/Makefile`, `module/Makefile`에서 구체적인 빌드 과정을 위임하여 실행하도록 구성합니다.
  - “관심사의 분리(Separation of Concerns)”를 통해 유지보수성을 높이고,  
    빌드 단계를 모듈화하여 확장성을 고려합니다.

- **기술 도전(메모리 동적 할당, ioctl, Timer 등)**

  - `kmalloc()`을 통한 동적 메모리 할당, `ioctl()`을 활용한 유연한 드라이버/유저 공간 통신,  
    커널 타이머를 이용한 주기적 이벤트 관리 등을 통해 리눅스 커널 프로그래밍 역량을 강화합니다.
  - Poll/Blocking I/O, 인터럽트, 그리고 GPIO 제어 등 다양한 커널 영역 기능을 접목하여  
    리눅스 디바이스 드라이버의 활용 폭을 넓힙니다.

- **최소 기능 제품(MVP) + 서비스 아키텍처 설계**

  - **MVP (Minimum Viable Product)**: 최소 기능 제품을 우선적으로 구현하여, 빠르게 제품을 검증하고 필요한 기능을 점진적으로 추가합니다.
  - **서비스 아키텍처**: 사용자 인터페이스(UI)와 비즈니스 로직(서비스)을 분리하여, 확장성과 유지보수성을 고려한 구조를 설계합니다.

- **디렉터리 구조 설계**

  - `app`, `module`, `uapi`, `driver` 디렉터리로 구분하고,  
    각 디렉터리별로 담당하는 역할(응용프로그램, 커널 모듈, 헤더/공용 정의, 저수준 드라이버)을 명확히 합니다.

- **코드 재사용성/확장성 고려**
  - 핵심 로직을 라이브러리 형태로 추출하는 등 재사용성과 확장성을 높일 수 있는 코드 구조를 채택하여,  
     향후 다른 디바이스나 기능 추가 시 효율적으로 대응할 수 있도록 합니다.
    &nbsp;
