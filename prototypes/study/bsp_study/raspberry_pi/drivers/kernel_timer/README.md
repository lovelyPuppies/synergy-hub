...
1. **크로스 컴파일 환경 구축**
   - LLVM/clang 환경 설정을 통한 임베디드/크로스 환경에서 빌드 및 배포 용이성 확보.
     - 빠른 컴파일 속도, 정확한 에러 메시지, 더 나은 최적화

- **디렉터리 구조 설계**

  - `app`, `module`, `uapi`, `driver` 디렉터리로 역할 분리

    **각 디렉터리의 책임 명확화**: (응용프로그램, 커널 모듈, 헤더/공용 정의, 저수준 드라이버)
...
- **최소 기능 제품(MVP)**
  ...
  - **MVP (Minimum Viable Product)**: 최소 기능 제품을 우선적으로 구현하여, 빠르게 제품을 검증하고 필요한 기능을 점진적으로 추가가 가능하도록 함.
