...
static ssize_t ledkeydev_read(struct file *filp, char *buf, size_t count,
                              loff_t *f_pos) {
  //	char kbuf;
  int ret;
#if DEBUG
...
static ssize_t ledkeydev_write(struct file *filp, const char *buf, size_t count,
                               loff_t *f_pos) {
  char kbuf;
  int ret;
#if DEBUG
...
static long ledkeydev_ioctl(struct file *filp, unsigned int cmd,
                            unsigned long arg) {
...
static unsigned int ledkeydev_poll(struct file *filp,
...
static ssize_t proc_read(struct file *filp, char *buf, size_t count,
                         loff_t *f_pos) {
  char kbuf[10];
  int ret;
  int len = 0;
...
static ssize_t proc_write(struct file *filp, const char *buf, size_t count,
                          loff_t *f_pos) {
  char kbuf[10];
  int ret;
  //	get_user(kbuf,buf);
...
static void mkproc(void) {
  ...
  keyledproc_led_fp =
      proc_create_data("led", S_IFREG | S_IRWXU | S_IRWXG | S_IRWXO,
                       keyledproc_root_fp, &proc_led_fops, NULL);
  keyledproc_key_fp =
      proc_create_data("key", S_IFREG | S_IRWXU | S_IRWXG | S_IRWXO,
                       keyledproc_root_fp, &proc_key_fops, NULL);
}
...
static int ledkeydev_init(void) {
  ...
  for (i = 0; i < GPIOKEYCNT; i++) {
    result = request_irq(sw_irq[i], sw_isr, IRQF_TRIGGER_RISING,
                         gpioName(key, i), NULL);
    if (result) {
