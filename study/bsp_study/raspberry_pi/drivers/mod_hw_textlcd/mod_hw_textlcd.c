...
static void _set_command_data(u_int8_t command_data,
                              int flags /*0:cmd, 1:data*/) {
  int i;
...
void textlcd_set_home(void) { _set_command_data(0x02, 0); }

EXPORT_SYMBOL_GPL(textlcd_set_home);

void textlcd_clear(void) { _set_command_data(0x01, 0); }
...
static ssize_t textlcd_line_0_read(struct file *file, char *buf, size_t count,
                                   loff_t *pos) {
  return simple_read_from_buffer(buf, count, pos, textlcd_line_buf[0],
                                 MAX_DATA_LEN);
}

static ssize_t textlcd_line_0_write(struct file *file, const char *buf,
...
static ssize_t textlcd_line_1_read(struct file *file, char *buf, size_t count,
                                   loff_t *pos) {
  return simple_read_from_buffer(buf, count, pos, textlcd_line_buf[1],
                                 MAX_DATA_LEN);
}

static ssize_t textlcd_line_1_write(struct file *file, const char *buf,
                                    size_t count, loff_t *pos) {
  int ret;
...
static int mod_hw_textlcd_init(void) {
  ...
  for (i = 0; i < ARRAY_SIZE(dentry_textlcd_line); ++i) {
    dentry_textlcd_line[i] =
        debugfs_create_file(file_name[i], 0644, NULL, NULL, &textlcd_fops[i]);
